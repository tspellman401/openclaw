...
function projectCodexDynamicTools(tools: readonly AnyAgentTool[]): {
  ...
  for (const entry of readCodexDynamicToolEntries(tools)) {
    ...
    const nameProjection = nameRead.readable
      ? projectCodexDynamicToolName(nameRead.value)
      : undefined;
    const diagnosticToolName = nameProjection?.toolName ?? `tool[${entry.toolIndex}]`;
    const nameViolation = !nameRead.readable
      ? `${diagnosticToolName}.name is unreadable`
      : nameProjection?.violation
        ? `${diagnosticToolName}.name ${nameProjection.violation}`
        : undefined;
    if (nameViolation) {
...
function projectCodexDynamicToolName(
  value: unknown,
): { toolName: string; violation?: undefined } | { toolName?: undefined; violation: string } {
  if (typeof value !== "string" || value.length === 0) {
    return { violation: "must be a non-empty string" };
  }
